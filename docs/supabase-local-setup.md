...
In local development, Studio is accessible at `http://localhost:54323` without authentication. No password configuration is needed.
...
SQL migrations live in `supabase/migrations/`. Naming convention uses UTC timestamps, auto-generated by the CLI:
...
```
20260306120000_initial_schema.sql
20260307090000_add_users_table.sql
```

Always use `supabase migration new` to create migrations so the timestamp prefix is consistent.
