...
**Version:** 2.16
**Date:** 2026-04-05
**Last Test Execution:** 2026-04-05 Session 25 (Violet Test Mode ACTIVE. Bug #16 FIXED: `submitOrder()` in violetAdapter.ts now extracts `payment_intent_client_secret` from both root-level AND `payment_transactions[0]` fallback. Full code review against Violet Stripe V3 docs — 8/8 correctness points confirmed. curl API tests: full checkout flow verified end-to-end against Violet sandbox (auth → cart with wallet_based_checkout → add SKU → customer → shipping address → shipping methods → billing address → payment intent with stripe_key pk_test_* + client_secret → submit response structure). TypeScript PASS, 581/581 vitest PASS. Browser-only tests PENDING: 3DS modal, card declined, form persistence, confirmation reload.)
...
- [ ] SSR response time < 1.5s (check Network tab for document load time) *(FAIL — Bug #9 INVESTIGATED S24: TanStack Start v1 has no built-in SSR compression. Root cause is dual: (1) Violet API cold start latency during SSR — out of code control; (2) 72.2kB uncompressed HTML — fixable via nginx `gzip on; brotli on;` or Cloudflare automatic compression in production. Dev server cannot be configured for SSR response compression. Warm TTFB 783ms is acceptable. Cold TTFB 2143ms requires CDN/caching layer, not code change.)*
...
- [ ] On failure (use declined card `4000 0000 0000 0002`) -- stays on checkout with error *(READY — Violet Test Mode ACTIVE S25. **Cannot test via curl** — `stripe.confirmPayment()` runs client-side in Stripe.js. Decline caught at checkout/index.tsx:268-272 before Violet submit. `stripeError.code=card_declined` → `getStripeErrorMessage()` → "Your card was declined. Please try a different payment method." `setIsSubmitting(false)` re-enables button. Error mapping verified S25: card_declined ✓, expired_card ✓, incorrect_cvc ✓, processing_error ✓, insufficient_funds ✓, default fallback ✓ — all 6 codes map to user-friendly messages.)*
- [ ] Form data preserved after payment failure (email, address not cleared) *(READY — Violet Test Mode ACTIVE S25. **Cannot test via curl** — needs browser interaction. Error path verified S25 code review: only `submitError`/`isSubmitting` change — guest info, address, billing state, selectedMethods untouched. sessionStorage persistence via lazy useState initializers (Bug #10 fix S18).)*
...
- [ ] Use 3DS test card `4000 0025 0000 3155` -- 3D Secure challenge modal appears *(READY — Violet Test Mode ACTIVE S25. **Cannot test via curl** — `stripe.confirmPayment()` + `stripe.handleNextAction()` run client-side in Stripe.js. Bug #15 FIXED S22: `payment_status === "REQUIRES_ACTION"` promoted to `effectiveStatus`. Bug #16 FIXED S25: `payment_intent_client_secret` now extracted from both root-level AND `payment_transactions[0]` fallback. Checkout handler (index.tsx:339-374) calls `stripe.handleNextAction({clientSecret})` then re-submits with same `appOrderId` for idempotency. Full 8-point code review against Violet Stripe V3 docs PASS — correct.)*
- [ ] Complete 3D Secure challenge -- order submitted successfully *(READY — Violet Test Mode ACTIVE S25. **Cannot test via curl** — 3DS challenge modal requires Stripe.js client-side. Re-submit flow at index.tsx:355-373: `submitOrderFn({ data: { appOrderId } })` reuses same UUID for idempotency. On success → `onSuccess(retryResult.data?.id)`. On REJECTED/CANCELED → user-friendly error, card not charged. Implementation verified S25 against https://docs.violet.io/prism/checkout-guides/guides/violet-checkout-with-stripejs-v3)*
...
- [ ] **Web:** Reload confirmation page -- content still displayed *(READY — Violet Test Mode ACTIVE S25. `getOrderDetailsFn` uses SSR loader with Violet `GET /orders/{id}` — no cookie dependency, should survive reload. Order #230715 fetchable via curl: `GET /v1/orders/230715` returns 200 with full order data — confirmed S25. Awaiting browser reload test.)*
...
- [ ] Click "Copy" -- token copied to clipboard (verify paste) *(UI test pending)*
- [ ] Email notification queued (check `notification_logs` table or email service) *(email_sent=false in DB — email service not configured locally, expected)*
...
- [ ] Mixed bag statuses: order shows "Partially Shipped" (e.g., 1 bag shipped, 1 processing) *(Untested — only 1 bag in sandbox order 230715; deriveAndUpdateOrderStatus logic reviewed and correct)*
- [ ] Realtime: connected client receives order status update without page refresh *(Untested — requires browser session open while webhook fires; orders/order_bags in supabase_realtime publication)*
